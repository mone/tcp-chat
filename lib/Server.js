  ...
  Server.prototype = {
    ...
    start: function(serverSocket) {
      ...
      this.server.on("connection", function(socket) {
        ...
        var clientId = that.processId+"-"+that.nextClientId++;
        log.info(that.processId + "] Client connected from " + socket.remoteAddress+":"+socket.remotePort + " new client id is " + clientId);

        var newClient = new Client(clientId, socket, that);
        that.clients[clientId] = newClient;
      });
